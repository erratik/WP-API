...
- Move the JavaScript client library into the plugin.
  ...
  Previously, the `cutv-api.js` file was a separate repository. The JavaScript
...
- Update the cookie authentication documentation examples.
  ...
  With 1.1 the localized JavaScript object for `cutv-api.js` changed to
...
- Update documentation link references to cutv-api.org.
...
- Include new client JS from github.io
  ...
  `cutv-api.github.io` to ensure it is always up-to-date.
...
- Add Backbone-based models and collections - These are available to your code
	by declaring a dependency on `cutv-api` ([#270][])
