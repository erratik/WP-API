...
class CUTV_REST_Snaptubes_Controller extends CUTV_REST_Controller {

	protected $post_type;

	public function __construct( $post_type ) {
		$this->post_type = 'videogallery';
	 ...
	 *
	 * @param  CUTV_REST_Request $request Full details about the request.
	 ...
	 *
	 * @param CUTV_REST_Request $request Full details about the request.
	 * @return WP_Error|CUTV_REST_Response
	 */
...
//        $args['post_type'] = ! isset( $request['search'] ) ? $request['search'] : $this->post_type;
		 ...
		 * @param CUTV_REST_Request $request The request used.
		 */
	...
	public function create_item( $request ) {
		 ...
		 * @param CUTV_REST_Request $request   Request object.
	...
	public function update_item( $request ) {
		...
		/* This action is documented in lib/endpoints/class-cutv-rest-controller.php */
	 ...
	 *
	 * @param CUTV_REST_Request $request Full details about the request.
	 * @return CUTV_REST_Response|WP_Error
	 */
	public function delete_item( $request ) {
		 ...
		 * @param CUTV_REST_Response $response The response data.
		 * @param CUTV_REST_Request  $request  The request sent to the API.
		 */
	 ...
	 * @param  CUTV_REST_Request $request The request object with post and terms data.
	 ...
	 * @param CUTV_REST_Request $request Request object.
	 * @return CUTV_REST_Response $data
	 */
	public function prepare_item_for_response( $post, $request ) {
		 ...
		 *
		 * @param CUTV_REST_Response   $response   The response object.
		 * @param WP_Post            $post       Post object.
		 * @param CUTV_REST_Request    $request    Request object.
		 */
	...
	protected function prepare_links( $post ) {
		...
		$links = array(
			...
			'about'      => array(
				'href'   => rest_url( '/cutv/v2/types/' . $this->post_type ),
			),
		);
		...
		if ( ( in_array( $post->post_type, array( 'post', 'page' ) ) || post_type_supports( $post->post_type, 'author' ) )
			...
			$links['author'] = array(
				'href'       => rest_url( '/cutv/v2/users/' . $post->post_author ),
				'embeddable' => true,
			);
		};
		...
		if ( $featured_media = get_post_thumbnail_id( $post->ID ) ) {
			$image_url = rest_url( 'cutv/v2/media/' . $featured_media );
