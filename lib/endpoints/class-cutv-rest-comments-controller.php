 ...
 * Access comments
 */
class CUTV_REST_Comments_Controller extends CUTV_REST_Controller {
	...
	public function get_items( $request ) {
		 ...
		 * @param CUTV_REST_Request $request       The current request.
		 */
	...
	public function create_item( $request ) {
		 ...
		 * @param CUTV_REST_Request $request          Request used to insert the comment.
		 */
	...
	public function update_item( $request ) {
		...
		/* This action is documented in lib/endpoints/class-cutv-rest-comments-controller.php */
	...
	public function delete_item( $request ) {
		 ...
		 * @param CUTV_REST_Response $response The response returned from the API.
		 * @param CUTV_REST_Request  $request  The request sent to the API.
		 */
	 ...
	 * @return CUTV_REST_Response $response
	 */
	public function prepare_item_for_response( $comment, $request ) {
		 ...
		 * @param CUTV_REST_Request   $request    Request used to generate the response.
		 */
	...
	protected function prepare_links( $comment ) {
		...
		if ( 0 !== (int) $comment->user_id ) {
			$links['author'] = array(
				'href'       => rest_url( 'cutv/v2/users/' . $comment->user_id ),
				'embeddable' => true,
			);
		}
		...
		if ( 0 !== (int) $comment->comment_post_ID ) {
			...
			if ( ! empty( $post->ID ) ) {
				...
				$links['up'] = array(
					'href'       => rest_url( 'cutv/v2/' . $base . '/' . $comment->comment_post_ID ),
					'embeddable' => true,
	...
	protected function check_read_post_permission( $post ) {
		$posts_controller = new CUTV_REST_Posts_Controller( $post->post_type );
