...
class WP_Test_rest_Post_Statuses_Controller extends WP_Test_rest_Controller_Testcase {
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/cutv/v2/statuses', $routes );
		$this->assertArrayHasKey( '/cutv/v2/statuses/(?P<status>[\w-]+)', $routes );
	}

	public function test_context_param() {
		// Collection
		$request = new CUTV_REST_Request( 'OPTIONS', '/cutv/v2/statuses' );
		$response = $this->server->dispatch( $request );
		$data = $response->get_data();
		$this->assertEquals( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertEqualSets( array( 'embed', 'view', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
		// Single
		$request = new CUTV_REST_Request( 'OPTIONS', '/cutv/v2/statuses/publish' );
		$response = $this->server->dispatch( $request );
		$data = $response->get_data();
		$this->assertEquals( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertEqualSets( array( 'embed', 'view', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/statuses' );
		$response = $this->server->dispatch( $request );
	...
	public function test_get_item() {
		$user_id = $this->factory->user->create( array( 'role' => 'author' ) );
		wp_set_current_user( $user_id );
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/statuses/publish' );
		$request->set_param( 'context', 'edit' );
		$response = $this->server->dispatch( $request );
	...
	public function test_get_item_invalid_status() {
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/statuses/invalid' );
		$response = $this->server->dispatch( $request );
	...
	public function test_get_item_invalid_access() {
		...
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/statuses/draft' );
		$response = $this->server->dispatch( $request );
	...
	public function test_get_item_invalid_internal() {
		...
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/statuses/inherit' );
		$response = $this->server->dispatch( $request );
	...
	public function test_prepare_item() {
		...
		$endpoint = new CUTV_REST_Post_Statuses_Controller;
