...
/**
 * Unit tests covering CUTV_REST_Terms_Controller functionality, used for Categories.
 *
 * @package WordPress
 * @subpackage JSON API
 */
class WP_Test_rest_Categories_Controller extends WP_Test_rest_Controller_Testcase {
	...
	public function test_register_routes() {
		...
		$this->assertArrayHasKey( '/cutv/v2/categories', $routes );
		$this->assertArrayHasKey( '/cutv/v2/categories/(?P<id>[\d]+)', $routes );
	}

	public function test_context_param() {
		// Collection
		$request = new CUTV_REST_Request( 'OPTIONS', '/cutv/v2/categories' );
		...
		$request = new CUTV_REST_Request( 'OPTIONS', '/cutv/v2/categories/' . $category1 );
		$response = $this->server->dispatch( $request );
		$data = $response->get_data();
		$this->assertEquals( 'view', $data['endpoints'][0]['args']['context']['default'] );
		$this->assertEqualSets( array( 'view', 'embed', 'edit' ), $data['endpoints'][0]['args']['context']['enum'] );
	...
	public function test_get_items() {
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/categories' );
		$response = $this->server->dispatch( $request );
	...
	public function test_get_items_custom_tax_post_args() {
		...
		$controller = new CUTV_REST_Terms_Controller( 'batman' );
		...
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/batman' );
		$request->set_param( 'post', $post_id );
		$response = $this->server->dispatch( $request );
		$this->assertEquals( 200, $response->get_status() );
	...
	public function test_get_terms_private_taxonomy() {
		...
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/terms/robin' );
		$response = $this->server->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}

	public function test_get_terms_invalid_taxonomy() {
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/invalid-taxonomy' );
		$response = $this->server->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}
	...
	public function test_get_item() {
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/categories/1' );
		$response = $this->server->dispatch( $request );
		$this->check_get_taxonomy_term_response( $response );
	}

	public function test_get_term_invalid_taxonomy() {
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/invalid-taxonomy/1' );
		$response = $this->server->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}

	public function test_get_term_invalid_term() {
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/categories/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = $this->server->dispatch( $request );
	...
	public function test_get_term_private_taxonomy() {
		...
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/terms/robin/' . $term1 );
		$response = $this->server->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}
	...
	public function test_get_item_incorrect_taxonomy() {
		...
		$term1 = $this->factory->term->create( array( 'name' => 'Cape', 'taxonomy' => 'robin' ) );
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/categories/' . $term1 );
		$response = $this->server->dispatch( $request );
		$this->assertErrorResponse( 'rest_term_invalid', $response, 404 );
	}
	...
	public function test_create_item() {
		...
		$request = new CUTV_REST_Request( 'POST', '/cutv/v2/categories' );
		...
		$headers = $response->get_headers();
		$data = $response->get_data();
		$this->assertContains( '/cutv/v2/categories/' . $data['id'], $headers['Location'] );
	...
	public function test_create_item_invalid_taxonomy() {
		wp_set_current_user( $this->administrator );
		$request = new CUTV_REST_Request( 'POST', '/cutv/v2/invalid-taxonomy' );
	...
	public function test_create_item_invalid_parent() {
		...
		$request = new CUTV_REST_Request( 'POST', '/cutv/v2/categories/' . $term->term_id );
		$request->set_param( 'name', 'My Awesome Term' );
	...
	public function test_update_item_invalid_taxonomy() {
		wp_set_current_user( $this->administrator );
		$request = new CUTV_REST_Request( 'POST', '/cutv/v2/invalid-taxonomy/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$request->set_param( 'name', 'Invalid Taxonomy' );
		$response = $this->server->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}
	...
	public function test_update_item_invalid_term() {
		wp_set_current_user( $this->administrator );
		$request = new CUTV_REST_Request( 'POST', '/cutv/v2/categories/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
	...
	public function test_update_item_parent() {
		...
		$request = new CUTV_REST_Request( 'POST', '/cutv/v2/categories/' . $term->term_taxonomy_id );
		$request->set_param( 'parent', $parent->term_id );
		$response = $this->server->dispatch( $request );
		$this->assertEquals( 200, $response->get_status() );
	...
	public function test_delete_item() {
		...
		$request = new CUTV_REST_Request( 'DELETE', '/cutv/v2/categories/' . $term->term_id );
	...
	public function test_delete_item_invalid_taxonomy() {
		wp_set_current_user( $this->administrator );
		$request = new CUTV_REST_Request( 'DELETE', '/cutv/v2/invalid-taxonomy/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = $this->server->dispatch( $request );
		$this->assertErrorResponse( 'rest_no_route', $response, 404 );
	}
	...
	public function test_delete_item_invalid_term() {
		wp_set_current_user( $this->administrator );
		$request = new CUTV_REST_Request( 'DELETE', '/cutv/v2/categories/' . REST_TESTS_IMPOSSIBLY_HIGH_NUMBER );
		$response = $this->server->dispatch( $request );
		$this->assertErrorResponse( 'rest_term_invalid', $response, 404 );
	}
	...
	public function test_prepare_taxonomy_term_child() {
		...
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/categories/' . $child );
		$response = $this->server->dispatch( $request );
		$data = $response->get_data();
		...
		$this->assertEquals( rest_url( 'cutv/v2/categories/1' ), $links['up'][0]['href'] );
	}
	...
	public function test_get_additional_field_registration() {
		...
		$request = new CUTV_REST_Request( 'GET', '/cutv/v2/categories/' . $category_id );
	...
	protected function check_taxonomy_term( $term, $data, $links ) {
		...
		$this->assertContains( 'cutv/v2/taxonomies/' . $term->taxonomy, $links['about'][0]['href'] );
		$this->assertEquals( add_query_arg( 'categories', $term->term_id, rest_url( 'cutv/v2/posts' ) ), $links['https://api.w.org/post_type'][0]['href'] );
	}
